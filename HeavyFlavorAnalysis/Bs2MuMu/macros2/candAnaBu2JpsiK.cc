...
void candAnaBu2JpsiK::candAnalysis() {
  ...
  fKa1MuMatch = doTriggerMatching(pk, true); // see if it matches fired HLT muon 

  if(1) { // for testing d.k.

    double mva=0;
    fKa1Missid2 = mvaMuon(pk,mva);  // true for tight  muons     
    fKa1MuMatch2 = doTriggerMatching(pk,false); // see if it matches Jpsi-disp HLT muon 


    ftmp7=ftmp8=ftmp9=0.;  // hlt pt and eta (class variable)
    ftmp11=0;
    fKa1MuMatchR = doTriggerMatchingR(pk,false); // matches to Bs/Jpsi-disp HLT muon 
    //cout<<" 1 "<<ftmp7<<" "<<ftmp8<<" "<<ftmp9<<" "<<ftmp11<<" "<< fKa1MuMatchR <<endl;
    fHltMu3Pt  = ftmp7; fHltMu3Eta = ftmp8; fHltMu3Phi = ftmp9; fHltMu3Id = ftmp11;

    ftmp7=ftmp8=ftmp9=0.; ftmp11=0;
    fKa1MuMatchR2 = doTriggerMatchingR(pk,true); // matches to fired HLT muon 
    //cout<<" 2 "<<ftmp7<<" "<<ftmp8<<" "<<ftmp9<<" "<<ftmp11<<" "<< fKa1MuMatchR2<<endl;
    //fHltMu3Pt  = ftmp7; fHltMu3Eta = ftmp8; fHltMu3Phi = ftmp9; fHltMu3Id = ftmp11;


    ftmp7=ftmp8=ftmp9=0.; ftmp11=0;

    //cout<<" call 1"<<" event "<<fEvt<<" "<<fpCand<<" "<<pk->fIndex<<endl;
    fKa1MuMatchR5 = doTriggerMatchingR_OLD(pk,false,-1,-1); // matches to any L3MuCandidate
    //cout<<" 5 "<<ftmp7<<" "<<ftmp8<<" "<<ftmp9<<" "<<ftmp11<<" "<< fKa1MuMatchR5<<endl;
    fHltMu4Pt  = ftmp7; fHltMu4Eta = ftmp8; fHltMu4Phi = ftmp9; fHltMu4Id = ftmp11;
    
    fKa1MuMatchR3 = matchToMuon(pk,true); // matches muon, ignore self muon 
    //if(fKa1Missid) cout<<"missid "<<fKa1Mssid<<" "<<fKa1MuMatch<<endl;


    //if(fKa1MuMatchR10 != fKa1MuMatchR5)  
    //cout<<fHltMu1Id<<" "<<fHltMu2Id<<" "<<fKa1MuMatchR10<<" "<<fKa1MuMatchR5<<endl;
  ...
  if(fPreselection) { // special misid tests d.k.

    //cout<<" call 2 "<<fpMuon1->fIndex<<" "<<fpMuon2->fIndex<<" "<<fEvt<<endl;
    fKa1MuMatchR10 = doTriggerMatchingR_OLD(pk,false, fHltMu1Id,fHltMu2Id); // matches to any L3MuCandidate excluding jpsi pair
    ...
    TVector3 muonMom;
    muonMom = fpMuon1->fPlab;  // mu1 from jpsi
    double dR1 = muonMom.DeltaR(trackMom);
    muonMom = fpMuon2->fPlab; // mu2 from jpsi
    double dR2 = muonMom.DeltaR(trackMom);

    // match muons from jpsi (redundant test)
    fKa1MuMatchR7 = doTriggerMatchingR(fpMuon1,false); // matches to Bs/Jpsi-disp HLT muon 
    fKa1MuMatchR8 = doTriggerMatchingR(fpMuon1,true);  // matches to fired HLT muon 

    fKa1MuMatchR9 = doTriggerMatchingR(fpMuon2,false); // matches to Bs/Jpsi-disp HLT muon 
    //fKa1MuMatchR10= doTriggerMatchingR(fpMuon2,true);  // matches to fired HLT muon 
...
void candAnaBu2JpsiK::moreReducedTree(TTree *t) {
  ...
  if(1) { // testing d.k.
    ...
    t->Branch("k1mumatchr8", &fKa1MuMatchR8,    "k1mumatchr8/F");
    t->Branch("k1mumatchr9", &fKa1MuMatchR9,    "k1mumatchr9/F");
    t->Branch("k1mumatchr10",&fKa1MuMatchR10,   "k1mumatchr10/F");

    t->Branch("hm3pt",    &fHltMu3Pt,      "hm3pt/D");
    t->Branch("hm3eta",   &fHltMu3Eta,     "hm3eta/D");
    t->Branch("hm4pt",    &fHltMu4Pt,      "hm4pt/D");
    t->Branch("hm4eta",   &fHltMu4Eta,     "hm4eta/D");
    t->Branch("hm3phi",   &fHltMu3Phi,     "hm3phi/D");
    t->Branch("hm4phi",   &fHltMu4Phi,     "hm4phi/D");
    t->Branch("hm3id",    &fHltMu3Id,      "hm3id/I");
    t->Branch("hm4id",    &fHltMu4Id,      "hm4id/I");
