...
void candAna::evtAnalysis(TAna01Event *evt) {
  ...
  //bool select_print = false; //dk
  //if(fEvt>4500000 && fEvt<5500000) {cout<<" selected "<<endl; select_print = true;} //dk
  //if(fRun==201671 && fEvt==249102695) { select_print = true;} //dk
  //if(fRun==201671 && fLS == 177) { select_print = true;} //dk
...
//   if(!selected) return;
  ...
  for (int iC = 0; iC < fpEvt->nCands(); ++iC) {
    ...
    // Move muon matching to candAnalysis to be able to use for K matching
 
//     if(fpMuon1 != NULL && fpMuon2 != NULL) { // do only when 2 muons exist

//       fHltMu1Pt = fHltMu1Eta = fHltMu1Phi = fHltMu2Pt  = fHltMu2Eta = fHltMu2Phi = fHltMu1Id = fHltMu2Id = -1;

//       //cout<<" do trigger matching both muons true"<<endl;
...
//       //cout<<" 1 "<<fHltMu1Pt<<" "<<fHltMu1Eta<<" "<<fHltMu1Phi<<" "<<fHltMu2Pt<<" "<<fHltMu2Eta<<" "
//       //  <<fHltMu2Phi<<" "<<fHltMu1Id<<" "<<fHltMu2Id << endl;

//       fHltMu1Pt = fHltMu1Eta = fHltMu1Phi = fHltMu2Pt  = fHltMu2Eta = fHltMu2Phi = fHltMu1Id = fHltMu2Id = -1;
//       cout<<" do trigger matching for pair : all"<<endl;
//       fHLTmatch3 = doTriggerMatching(fpMuon1, fpMuon2); // check matching for both muons in parallel (hlt fired)
...
//       //cout<<" pairs "<<fHltMu1Pt<<" "<<fHltMu1Eta<<" "<<fHltMu1Phi<<" "<<fHltMu2Pt<<" "<<fHltMu2Eta<<" "
//       //  <<fHltMu2Phi<<" "<<fHltMu1Id<<" "<<fHltMu2Id << endl;
//     }
...
void candAna::bookHist() {
  ...
  h11 = new TH1D("test1", "test1",100, 0., 100.); 
  h11 = new TH1D("test2", "test2",100, 0., 100.); 
  h11 = new TH1D("test3", "test3",100, 0., 100.); 
  h11 = new TH1D("test4", "test4",100, 0., 100.); 
  h11 = new TH1D("test5", "test5",100, 0., 100.); 
  h11 = new TH1D("test6", "test6",100, 0., 100.); 
  h11 = new TH1D("test7", "test7",100, 0., 100.);
...
void candAna::setupReducedTree(TTree *t) {
  ...
  if(1) {
    ...
    t->Branch("hm1id",    &fHltMu1Id,      "hm1id/I");
    t->Branch("hm2id",    &fHltMu2Id,      "hm2id/I");
  }
}
...
// ----------------------------------------------------------------------
// Trigger matching of a muon pair (fired HLT)
//
bool candAna::doTriggerMatching(TAnaTrack *fp1, TAnaTrack *fp2) { 
  ...
  for(int i=0; i!=fpEvt->nTrgObj(); i++) { // loop over all objects
    ...
    //if(localPrint) {cout<<i; tto->dump();}

    int id1 = tto->fNumber;
    //cout <<" mu1 "<< i<<" "<<tto->fLabel << " "<<id1 << endl;
	...
	if (deltaR2<deltaRthrsh) {
	  ...
	  if ( deltaR1<deltaRmin1 && deltaR2<deltaRmin2 ) { // good match
	    ...
	    if( trigMatchDeltaPt1 < 0.15 && trigMatchDeltaPt2 < 0.15 ) {
	      ...
	      fHltMu2Pt  = tto2->fP.Perp();
	      fHltMu2Eta = tto2->fP.Eta();
	      fHltMu2Phi = tto2->fP.Phi();


	      // Check the object from the L3MuonsCandidate list
	      ftmp11=-1;
	      double r = doTriggerMatchingR_OLD(fp1,true);
	      if(r<0.02) fHltMu1Id = ftmp11;
	      ftmp11=-1;
	      r = doTriggerMatchingR_OLD(fp2,true);
	      if(r<0.02) fHltMu2Id = ftmp11;

	      //cout << " best match "
	      //   <<indx1<<" "<< deltaRmin1 << " "<<mu1match<<" "<<hlt1<<" "<<trigMatchDeltaPt1<<" "<<fHltMu1Id<<" "
	      //   <<indx2<<" "<< deltaRmin2 << " "<<mu2match<<" "<<hlt2<<" "<<trigMatchDeltaPt2<<" "<<fHltMu2Id<<endl;
...
//------------------------------------
// Checks both muons from the triggered dimuon. OLD
...
bool candAna::doTriggerMatching(bool anyTrig) {
  ...
  fHltMu1Pt  = ftmp7; // testing
  fHltMu1Eta = ftmp8;
  fHltMu1Phi = ftmp9;
  fHltMu1Id  = ftmp11;
  //cout<<" single1 "<<ftmp7<<" "<<ftmp8<<" "<<ftmp9<<" "<<ftmp11<<endl;

  double dR2 = doTriggerMatchingR(fpMuon2, anyTrig);
  fHltMu2Pt  = ftmp7; // testing
  fHltMu2Eta = ftmp8;
  fHltMu2Phi = ftmp9;
  fHltMu2Id  = ftmp11;
  //cout<<" single2 "<<ftmp7<<" "<<ftmp8<<" "<<ftmp9<<" "<<ftmp11<<endl;
...
// To match a single track to a trigger object (selected or all)
// pt - track
// anyTrig - if true use fired trigger object, false used the specific hlt object
...
//        ) selected = true;
  ...
  //cout<<" R "<<ftmp7<<" "<<ftmp8<<" "<<ftmp9<<" "<<ftmp11<<endl;
...
double candAna::doTriggerMatchingR_OLD(TAnaTrack *pt, bool anyTrig) {
  ...
  double ret  = doTriggerMatchingR_OLD(pt, anyTrig, -1, -1);
...
// To match a single track to a trigger object (selected or all)
// pt - track
// anyTrig - unused, now just to signal where it was called from 
double candAna::doTriggerMatchingR_OLD(TAnaTrack *pt, bool anyTrig, int hltid1, int hltid2) {
  fTrigMatchDeltaPt = 99.;

   const bool localPrint = false;
  //bool localPrint = select_print;

   //const bool anyTrig = true; // fix to anu Mu/mu
  ...
  for(int i=0; i!=fpEvt->nTrgObj(); i++) {
    ...
    if( (i==hltid1) || (i==hltid2) ) continue; // skip these hlt objects
...
//     } // allTrig
  ...
  //cout<<" OLD "<<ftmp7<<" "<<ftmp8<<" "<<ftmp9<<" "<<ftmp11<<endl;
  
  if (fVerbose > 15 || localPrint) 
    cout << "best trigger matching: " << mu1match << " dR: " << deltaRminMu1 << " "<<hlt1<<endl;
  
  if(anyTrig) { // for muons = true 
    ((TH1D*)fHistDir->Get("test6"))->Fill(float(count1)); ((TH1D*)fHistDir->Get("test7"))->Fill(float(count2));
    //cout<<" this is for muons "<<count1<<" "<<count2<<endl;
  } else {  // for kaons 
    if(hltid1==-1) {((TH1D*)fHistDir->Get("test1"))->Fill(float(count1)); ((TH1D*)fHistDir->Get("test2"))->Fill(float(count2));}
    else           {((TH1D*)fHistDir->Get("test4"))->Fill(float(count1)); ((TH1D*)fHistDir->Get("test5"))->Fill(float(count2));}
    //if(hltid1==-1) cout<<" this is for kaons 1 - "<<count1<<" "<<count2<<" "<<hltid1<<" "<<hltid2<<endl;
    //else           cout<<" this is for kaons 2 - "<<count1<<" "<<count2<<" "<<hltid1<<" "<<hltid2<<endl;
